...
3) A dataset of >18M (and counting) trajectories generated by self-play between agents.
...
2. **NeurIPS PokéAgent Challenge**: Basically restarted from scratch. Broadly speaking, we *reduced* model sizes, reward shaping, and the paper's emphasis on long-term memory while *improving* generalization over diverse team choices and prioritizing support for gen9ou. However, it took several iterations to recover the paper's Gen 1-4 performance.
